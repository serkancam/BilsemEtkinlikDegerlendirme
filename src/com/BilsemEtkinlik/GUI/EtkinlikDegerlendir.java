...
public class EtkinlikDegerlendir extends javax.swing.JInternalFrame {
    ...
    void EtkinligeAitSorulariGetir()
    {
        ...
        if(secilenSinif>0)
        {
            ...
            {
                eklenen[i]=new Cift2(soru.getSoruId(), soru.getSoru());
                //sorular cift2 olacak yapıldı
                boyut[i]=soru.getSoru().trim().length()*7+10;
                i++;
            }
            
            TabloIslemleri.KolonBoyutuAyarla(tblDegerlendirme, boyut);
        ...
        **/
        ...
        for(Ogrenciler ogrenci:ogrenciler)
        {
           ...
           // System.out.println("com.BilsemEtkinlik.GUI.EtkinlikDegerlendir.OgrencilereNotlariniTabloyaEkle()");
            ...
            for (EtkinlikNotlari nt:notlar) 
            {
                for (int i = 1; i < eklenen.length; i++) 
                {
                       if(((Cift2)model.getValueAt(0, i)).key==nt.getSoru().getSoruId())
                       {
                           eklenen[i]=nt.getPuan();
                       }
    ...
    void Raporlama()
    {
        ...
        String dosyaYolu="Raporlar/"+cbSiniflar.getSelectedItem().toString()+"/"+cbEtkinlikler.getSelectedItem().toString().split("-")[0]+"/";
        ...
        try
        {
            ...
            for (int t = 1; t < model.getRowCount(); t++) 
            {
               ...
               //cb.saveState();
                ...
                String[] etD=cbEtkinlikler.getSelectedItem().toString().split("-");
                String etkinlik="";
                for(int e=1;e<etD.length;e++)
                {
                    etkinlik=etD[e];
                }
                dosya.add(new Paragraph("Etkinliğin Adı:"+etkinlik,fontNormal));
                dosya.add(new Paragraph(""));
                
                String[] sinifD=cbSiniflar.getSelectedItem().toString().split("-");
                String sinif="";
                for(int s=1;s<sinifD.length;s++)
                {
                    sinif=sinifD[s]+"-";
                }
                sinif=sinif.substring(0, sinif.length()-1);
                dosya.add(new Paragraph("Sınıfı:"+sinif,fontNormal));
                dosya.add(new Paragraph(""));
               ...
               for (int i = 1; i < model.getColumnCount(); i++) 
                {
                    PdfPCell soruHucresi=new PdfPCell(new Paragraph(CENTER_ALIGNMENT,((Cift2)model.getValueAt(0, i)).value,fontNormal));
                    //soru burada alınıyor yapıldı
           ...
           lblRaporlamaBilgi.setText("<html>İşlem Bilgisi: Raporlama ilemi Tamamlandı.</html>");
    ...
    private void initComponents() {
        ...
        tblDegerlendirme.setAutoResizeMode(javax.swing.JTable.AUTO_RESIZE_OFF);
        ...
        btnPuanlariAktar.setText("<html>Paunları Kaydet ve Raporla</html>");
        btnPuanlariAktar.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnPuanlariAktarActionPerformed(evt);
            }
        });
        pnlSecim.add(btnPuanlariAktar, new org.netbeans.lib.awtextra.AbsoluteConstraints(330, 10, 180, 50));
        ...
        pnlSecim.add(lblBilgi, new org.netbeans.lib.awtextra.AbsoluteConstraints(330, 70, 230, 70));

        lblRaporlamaBilgi.setText("Raporlama Bilgisi");
        lblRaporlamaBilgi.setVerticalAlignment(javax.swing.SwingConstants.TOP);
        pnlSecim.add(lblRaporlamaBilgi, new org.netbeans.lib.awtextra.AbsoluteConstraints(580, 70, 250, 70));
        ...
        layout.setHorizontalGroup(
            ...
            .addComponent(pnlSecim, javax.swing.GroupLayout.DEFAULT_SIZE, 1340, Short.MAX_VALUE)
        );
        layout.setVerticalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(layout.createSequentialGroup()
                .addComponent(pnlSecim, javax.swing.GroupLayout.PREFERRED_SIZE, 170, javax.swing.GroupLayout.PREFERRED_SIZE)
    ...
    private void btnPuanlariAktarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPuanlariAktarActionPerformed
        ...
        /*for (int i = 1; i < model.getRowCount(); i++) 
        {
            for(int j=1;j<model.getColumnCount();j++)
            {
                ...
                if(  Integer.parseInt(model.getValueAt(i,j).toString().trim())>5 || Integer.parseInt(model.getValueAt(i,j).toString().trim())<1)
                {
                    lblRaporlamaBilgi.setText("<html>Raporlama Bilgisi:Notu 1-5 arasında olmayan öğrenci var.</html>");
                    islem1=false;
                    return;
                }
        ...
        if(etkinklikSoruSayisi>0 && model.getRowCount()>1)
        {
            for (int i = 1; i < model.getRowCount(); i++) 
            {
                ...
                for (int j = 1; j < model.getColumnCount(); j++) 
                {
                    ...
                    soruId=((Cift2)model.getValueAt(0, j)).key;//soru burada alınıyor yapıldı
                    
                    notlar=islem.OgrenciyeAitPuaniGetir(ogrenciNo, soruId);
                    if(notlar.size()>0 && (!(model.getValueAt(i, j).toString().trim()).isEmpty()))
                    {
                        //güncelle
                    ...
                    else if(notlar.isEmpty() && (!((model.getValueAt(i, j).toString().trim()).isEmpty())))
                    {
                        //ekle
